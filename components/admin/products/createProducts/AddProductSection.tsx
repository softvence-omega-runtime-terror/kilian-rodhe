...
import { useCreateProductMutation, ProductRequest } from "@/app/store/slices/services/adminService/products/createProductApi";
...
const AddNewProductScreen = ({ onViewChange }: { onViewChange: ViewChangeHandler }) => {
  ...
  const handleAction = async (action: "cancel" | "add") => {
    ...
    if (!productData.sub_category) validationErrors.push("Sub-category is required");
    if (!productData.classification) validationErrors.push("Classification is required");
    if (!productData.description.trim()) validationErrors.push("Description is required");
    if (!productData.price || Number(productData.price) <= 0) validationErrors.push("Valid price is required");
    if (!productData.stock || Number(productData.stock) < 0) validationErrors.push("Valid stock quantity is required");
    if (!productData.sku.trim()) validationErrors.push("SKU is required");
    if (productData.colors.length === 0) validationErrors.push("At least one color must be selected");
    if (productData.sizes.length === 0) validationErrors.push("At least one size must be selected");
    if (productData.images.length === 0) validationErrors.push("At least one product image is required");
